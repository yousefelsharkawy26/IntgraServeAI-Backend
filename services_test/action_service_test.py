...
# استيراد الخدمة والمتغيرات الافتراضية
...
def mock_settings(tmp_path, monkeypatch):
    ...
    # إنشاء كلاس وهمي يحتوي على الإعدادات التي نحتاجها فقط
    class MockSettings:
    ...
    # استبدال الـ settings الحقيقي بالكلاس الوهمي بالكامل داخل ملف action_service
...
# ============================================================================
# Tests: Initialization & File Operations
# ============================================================================
...
def test_initialization_creates_default_file(mock_settings):
    ...
    # التأكد من إضافة الأفعال الافتراضية
...
def test_validate_action_success(action_service):
    ...
    action_dict = {
        "name": "Test",
        "type": "api_request",
        "execution_config": {"url": "http://test.com", "method": "POST", "protocol": "http"},
        "response_config": {"mode": "json", "template": "Test", "on_error": "Error"}
    }
    
    # الدالة يجب أن تُرجع (True, [])
...
def test_validate_action_body_param_on_get(action_service):
    ...
    action_dict = {
        "name": "Test",
        "type": "api_request",
        "execution_config": {"url": "http://test.com", "method": "GET", "protocol": "http"},
        "parameters": {
            "data": {
                "param_type": "body",
                "type": "string",
...
def test_validate_action_invalid_protocol(action_service):
    ...
    action_dict = {
        "name": "Test",
        "type": "api_request",
        "execution_config": {"url": "http://test.com", "method": "GET", "protocol": "grpc"}
    }
    with pytest.raises(InvalidActionFieldException):
        action_service.validate_action(action_dict)

# ============================================================================
# Tests: CRUD Operations (Async)
# ============================================================================
...
async def test_create_internal_action_fails(action_service):
    ...
    # لاحظ أننا نستخدم هنا model_construct بدلاً من الاستدعاء العادي
    # هذا يسمح لنا بتخطي حماية Pydantic لاختبار حماية الخدمة (ActionService)
...
async def test_update_action(action_service, valid_api_action_data):
    ...
    # التحقق من أن الخصائص القديمة ما زالت موجودة
...
async def test_toggle_action_status(action_service):
    ...
    # الأفعال الداخلية تبدأ مفعلة (True)
    action_id = "INT-001"
...
# ============================================================================
# Tests: Backup Operations (Async)
# ============================================================================

@pytest.mark.asyncio
async def test_backup_created_on_save(action_service, valid_api_action_data, mock_settings):
    """اختبار أن إجراء تعديل (إنشاء فعل) يقوم بإنشاء ملف نسخة احتياطية"""
    assert not mock_settings["backup_dir"].exists() or len(list(mock_settings["backup_dir"].glob("*.json"))) == 0
    
    # هذه العملية ستقوم بالاستدعاء لـ _save_data والتي تنشئ الباك أب
